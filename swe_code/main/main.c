...
int main(int argc, char** argv) {
    ...
    for (attempt = 0; attempt < sim_params.nattempts; attempt++) {
        ...
        for (i = 1; i <= sim_params.nsteps; i++) {
            for (rk_val = 0; rk_val < 4; rk_val++) {
                if (sim_params.OCL == 1) {
...
void verify_output(fType* H1, fType* H2) {
    ...
    for (i = LPSMD->compute_pid_s; i < LPSMD->compute_pid_e; i++) {
        for (j = 0; j < 4; j++) {
