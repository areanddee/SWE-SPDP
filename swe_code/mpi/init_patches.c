...
patch_struct* get_GPD(GSMD_struct GSMD) {
  ...
  int nbr;
    ...
    for (rank = 0; rank < n_default; rank++) {
    ...
    for (rank = n_default; rank < mpi_size; rank++) {
    ...
    for (rank = 0; rank < mpi_size; rank++) {
        ...
        for (gid = GPD[rank].compute_gid_s; gid < GPD[rank].compute_gid_e; gid ++) {

            // iterate through each neighbor
            for (nbr = 0; nbr < GSMD.Nnbr; nbr++) {
